        ...
        {dkd_filtered_couriers.length === 0 ? <DkdEmptyState icon="account-search-outline" title={dkd_courier_search ? 'Kurye bulunamadı' : 'Henüz bağlı kurye yok'} text={dkd_courier_search ? 'Bu isim veya plakayla eşleşen bağlı kurye bulunamadı.' : 'Kurye Ekle ile DraBornGo kullanıcılarını işletmene bağlayabilirsin.'} /> : dkd_visible_couriers.map((dkd_courier) => {
          ...
          return <LinearGradient key={dkd_courier.dkd_courier_user_id} colors={dkd_courier.dkd_is_online ? ['#0A2631', '#111E3C', '#261B49'] : ['#111A2A', '#111827', '#17172D']} style={styles.courierCard}>
            ...
            <View style={styles.rateGrid}>
              <LinearGradient colors={['rgba(255,194,92,.17)', 'rgba(255,132,78,.07)']} style={styles.rateTile}><MaterialCommunityIcons name="package-variant-closed-check" size={20} color="#FFD071" /><Text style={styles.rateLabel} numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.72}>GÜNLÜK PAKET</Text><Text style={styles.rateValue} numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.8}>{dkd_counts_value.dkd_daily_packages ?? Number(dkd_courier?.dkd_today?.dkd_completed_jobs || 0)}</Text></LinearGradient>
              <LinearGradient colors={['rgba(91,222,255,.18)', 'rgba(70,124,255,.08)']} style={styles.rateTile}><MaterialCommunityIcons name="calendar-month-outline" size={20} color="#77EAFF" /><Text style={styles.rateLabel} numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.72}>AYLIK PAKET</Text><Text style={styles.rateValue} numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.8}>{dkd_counts_value.dkd_monthly_packages ?? 0}</Text></LinearGradient>
              <LinearGradient colors={['rgba(115,240,178,.18)', 'rgba(72,199,144,.07)']} style={styles.rateTile}><MaterialCommunityIcons name="cash-fast" size={20} color="#72F0B2" /><Text style={styles.rateLabel} numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.66}>BUGÜN KAZANÇ</Text><Text style={styles.rateValue} numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.68}>{dkd_money(dkd_courier.dkd_today?.dkd_earnings_tl)}</Text></LinearGradient>
            </View>
