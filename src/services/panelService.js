...
function dkd_safe_coordinate_number(dkd_value, dkd_axis_value) {
  ...
  if (Math.abs(dkd_number_value) < 0.0001) return null;
...
export async function dkd_panel_fetch_dashboard() {
  const { data, error } = await supabase.rpc('dkd_business_dashboard_dkd'); dkd_throw_rpc_error(error); return data || {};
}
export async function dkd_panel_fetch_orders(dkd_limit_value = 100) {
  const { data, error } = await supabase.rpc('dkd_business_orders_dkd', { dkd_param_limit: dkd_limit_value }); dkd_throw_rpc_error(error); return data?.dkd_orders || [];
}
export async function dkd_panel_fetch_couriers() {
  const { data, error } = await supabase.rpc('dkd_business_couriers_dkd'); dkd_throw_rpc_error(error); return data?.dkd_couriers || [];
}
export async function dkd_panel_search_couriers(dkd_query_value) {
  const { data, error } = await supabase.rpc('dkd_business_courier_search_dkd', { dkd_param_query: dkd_query_value }); dkd_throw_rpc_error(error); return data || [];
}
...
export async function dkd_panel_set_courier_max_hours(dkd_user_id_value, dkd_hours_value) {
  const { data, error } = await supabase.rpc('dkd_business_courier_max_hours_set_dkd', {
    dkd_param_courier_user_id: dkd_user_id_value,
    dkd_param_max_online_hours: Number(dkd_hours_value || 0),
  });
  dkd_throw_rpc_error(error); return data || {};
}
export async function dkd_panel_set_courier_online(dkd_user_id_value, dkd_online_value) {
  const { data, error } = await supabase.rpc('dkd_business_courier_online_set_dkd', {
    dkd_param_courier_user_id: dkd_user_id_value,
    dkd_param_online: dkd_online_value === true,
  });
  dkd_throw_rpc_error(error);
...
export async function dkd_panel_unlink_courier(dkd_user_id_value) {
  const { data, error } = await supabase.rpc('dkd_business_courier_unlink_dkd', { dkd_param_courier_user_id: dkd_user_id_value }); dkd_throw_rpc_error(error); return data;
}
export async function dkd_panel_fetch_courier_earnings(dkd_user_id_value) {
  const { data, error } = await supabase.rpc('dkd_courier_earnings_summary_dkd', { dkd_param_user_id: dkd_user_id_value }); dkd_throw_rpc_error(error); return data || {};
}
export async function dkd_panel_fetch_courier_report(dkd_user_id_value, dkd_day_value) {
  const { data, error } = await supabase.rpc('dkd_business_courier_report_dkd', { dkd_param_courier_user_id: dkd_user_id_value, dkd_param_day: dkd_day_value }); dkd_throw_rpc_error(error); return data || {};
}
export async function dkd_panel_fetch_day_report(dkd_day_value) {
  const { data, error } = await supabase.rpc('dkd_business_day_report_dkd', { dkd_param_day: dkd_day_value }); dkd_throw_rpc_error(error); return data || {};
}
...
export async function dkd_panel_create_order(dkd_form_value) {
  ...
  if ((dkd_dropoff_lat_value == null || dkd_dropoff_lng_value == null) && dkd_delivery_address_value) {
    const dkd_geocode_value = await dkd_panel_geocode_delivery_address_value(
      dkd_delivery_address_value,
      { dkd_city_value: dkd_form_value.city || 'Ankara' },
    ).catch(() => null);
  ...
  const { data, error } = await supabase.rpc('dkd_business_order_create_dkd', {
    dkd_param_order_ref: String(dkd_form_value.orderRef || '').trim(),
    dkd_param_title: String(dkd_form_value.title || 'Sipariş').trim(),
    dkd_param_customer_name: String(dkd_form_value.customerName || '').trim(),
    dkd_param_customer_phone: String(dkd_form_value.customerPhone || '').trim(),
    dkd_param_delivery_address: dkd_delivery_address_value,
    dkd_param_delivery_note: String(dkd_form_value.deliveryNote || '').trim(),
    dkd_param_customer_charge_tl: Number(String(dkd_form_value.amount || '0').replace(',', '.')) || 0,
    dkd_param_dropoff_lat: dkd_dropoff_lat_value,
    dkd_param_dropoff_lng: dkd_dropoff_lng_value,
  });
  dkd_throw_rpc_error(error);
  return data || {};
}
...
export function dkd_panel_subscribe_live(dkd_callback_value) {
  ...
  ['dkd_courier_jobs','dkd_courier_live_locations','dkd_business_couriers','dkd_courier_online_sessions'].forEach((dkd_table_value) => {
    dkd_channel_value = dkd_channel_value.on('postgres_changes', {
      event:'*', schema:'public', table:dkd_table_value,
    }, (dkd_payload_value) => {
