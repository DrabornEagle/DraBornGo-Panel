...
function dkd_mapbox_query_text_value(dkd_value) { return String(dkd_value || '').trim().replace(/\s+/g, ' '); }
function dkd_mapbox_feature_point_value(dkd_feature_value) {
  ...
  if (Array.isArray(dkd_geometry_value) && dkd_geometry_value.length >= 2) return dkd_panel_coordinate_value(dkd_geometry_value[1], dkd_geometry_value[0]);
  return null;
}
function dkd_mapbox_feature_text_value(dkd_feature_value) {
  const dkd_properties_value = dkd_feature_value?.properties || {};
  return [dkd_properties_value.name, dkd_properties_value.name_preferred, dkd_properties_value.full_address, dkd_properties_value.address, dkd_properties_value.place_formatted]
...
function dkd_mapbox_feature_score_value(dkd_feature_value, dkd_query_value) {
  ...
  const dkd_query_token_values = dkd_mapbox_query_text_value(dkd_query_value).toLocaleLowerCase('tr-TR').split(/[^0-9a-zçğıöşü]+/i).filter((dkd_token_value) => dkd_token_value.length >= 3);
  const dkd_generic_value = new Set(['ankara', 'turkiye', 'türkiye', 'mahallesi', 'mahalle', 'sokak', 'cadde', 'caddesi', 'avm', 'mall']);
  const dkd_important_values = dkd_query_token_values.filter((dkd_token_value) => !dkd_generic_value.has(dkd_token_value));
  const dkd_match_count_value = dkd_query_token_values.reduce((dkd_total_value, dkd_token_value) => dkd_total_value + (dkd_feature_text_value.includes(dkd_token_value) ? 1 : 0), 0);
  const dkd_important_match_value = dkd_important_values.reduce((dkd_total_value, dkd_token_value) => dkd_total_value + (dkd_feature_text_value.includes(dkd_token_value) ? 1 : 0), 0);
  ...
  const dkd_missing_important_penalty_value = Math.max(0, dkd_important_values.length - dkd_important_match_value) * 120;
  return dkd_match_count_value * 45 + dkd_important_match_value * 85 + dkd_type_bonus_value + dkd_ankara_bonus_value - dkd_missing_important_penalty_value;
}
...
export async function dkd_panel_geocode_delivery_address_value(dkd_address_value, dkd_options_value = {}) {
  ...
  const dkd_query_with_context_value = /\b(ankara|türkiye|turkiye)\b/i.test(dkd_query_value) ? dkd_query_value : [dkd_query_value, dkd_context_city_value, 'Türkiye'].filter(Boolean).join(', ');
  const dkd_parameters_value = new URLSearchParams({ access_token: dkd_mapbox_token_value, q: dkd_query_with_context_value, country: 'TR', language: 'tr', limit: '10', types: 'poi,address,street,neighborhood,district,place,locality', auto_complete: 'true' });
  const dkd_proximity_value = dkd_options_value?.dkd_proximity_value;
  const dkd_named_place_value = /\b(avm|mall|hastane|hospital|üniversite|universite|otel|hotel|market|restoran|restaurant)\b/i.test(dkd_query_value);
  if (!dkd_named_place_value && dkd_proximity_value?.dkd_coordinate_value) dkd_parameters_value.set('proximity', dkd_proximity_value.dkd_coordinate_value.join(','));
  ...
  const dkd_ranked_values = dkd_feature_values.map((dkd_feature_value) => ({ dkd_feature_value, dkd_point_value: dkd_mapbox_feature_point_value(dkd_feature_value), dkd_score_value: dkd_mapbox_feature_score_value(dkd_feature_value, dkd_query_value) })).filter((dkd_item_value) => dkd_item_value.dkd_point_value).sort((dkd_left_value, dkd_right_value) => dkd_right_value.dkd_score_value - dkd_left_value.dkd_score_value);
  const dkd_best_value = dkd_ranked_values[0];
  if (!dkd_best_value) return null;
  return { ...dkd_best_value.dkd_point_value, dkd_place_name_value: dkd_mapbox_feature_text_value(dkd_best_value.dkd_feature_value) || dkd_query_value, dkd_address_text_value: dkd_mapbox_feature_text_value(dkd_best_value.dkd_feature_value) || dkd_query_value, dkd_is_verified_place_value: false };
}
...
export async function dkd_panel_fetch_live_route_value(dkd_start_value, dkd_end_value) {
  ...
  if (!dkd_mapbox_token_value.startsWith('pk.')) {
    return { dkd_distance_km_value: dkd_straight_distance_value, dkd_duration_min_value: dkd_straight_distance_value == null ? null : Math.max(1, Math.round((dkd_straight_distance_value / 27) * 60 + 3)), dkd_route_coordinate_values: [dkd_start_value.dkd_coordinate_value, dkd_end_value.dkd_coordinate_value], dkd_is_fallback_value: true };
  }
  ...
  const dkd_parameters_value = new URLSearchParams({ access_token: dkd_mapbox_token_value, geometries: 'geojson', overview: 'full', steps: 'false' });
  try {
    ...
    return { dkd_distance_km_value: Number(dkd_route_value.distance || 0) / 1000, dkd_duration_min_value: Math.max(1, Number(dkd_route_value.duration || 0) / 60), dkd_route_coordinate_values: Array.isArray(dkd_route_value?.geometry?.coordinates) ? dkd_route_value.geometry.coordinates : [dkd_start_value.dkd_coordinate_value, dkd_end_value.dkd_coordinate_value], dkd_is_fallback_value: false, dkd_route_created_at_value: Date.now() };
  } catch {
    return { dkd_distance_km_value: dkd_straight_distance_value, dkd_duration_min_value: dkd_straight_distance_value == null ? null : Math.max(1, Math.round((dkd_straight_distance_value / 27) * 60 + 3)), dkd_route_coordinate_values: [dkd_start_value.dkd_coordinate_value, dkd_end_value.dkd_coordinate_value], dkd_is_fallback_value: true, dkd_route_created_at_value: Date.now() };
  }
}

export function dkd_panel_live_route_progress_value(dkd_route_value, dkd_current_point_value) {
  const dkd_coordinate_values = Array.isArray(dkd_route_value?.dkd_route_coordinate_values) ? dkd_route_value.dkd_route_coordinate_values : [];
  if (!dkd_current_point_value || dkd_coordinate_values.length < 2) return null;
  const dkd_point_values = dkd_coordinate_values.map((dkd_coordinate_value) => dkd_panel_coordinate_value(dkd_coordinate_value?.[1], dkd_coordinate_value?.[0])).filter(Boolean);
  ...
  dkd_point_values.forEach((dkd_point_value, dkd_index_value) => {
    ...
    if (Number.isFinite(dkd_distance_value) && dkd_distance_value < dkd_nearest_distance_value) { dkd_nearest_distance_value = dkd_distance_value; dkd_nearest_index_value = dkd_index_value; }
  });
  ...
  const dkd_initial_distance_value = Number(dkd_route_value?.dkd_distance_km_value || 0);
  const dkd_initial_duration_value = Number(dkd_route_value?.dkd_duration_min_value || 0);
  ...
  const dkd_remaining_min_value = dkd_initial_duration_value > 0 ? Math.max(1, dkd_initial_duration_value * dkd_ratio_value) : Math.max(1, (dkd_remaining_km_value / 27) * 60 + 2);
  return { dkd_distance_km_value: Math.max(0, dkd_remaining_km_value), dkd_duration_min_value: Math.max(1, dkd_remaining_min_value), dkd_nearest_route_index_value: dkd_nearest_index_value };
}

export function dkd_panel_route_geojson_value(dkd_coordinate_values) {
  return { type: 'Feature', properties: {}, geometry: { type: 'LineString', coordinates: Array.isArray(dkd_coordinate_values) ? dkd_coordinate_values : [] } };
}
