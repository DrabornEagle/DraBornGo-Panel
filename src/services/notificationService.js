...
function dkd_remember_notification_key_value(dkd_key_value) {
  ...
  if (!dkd_safe_key_value) return false;
  if (dkd_job_notification_seen_value.has(dkd_safe_key_value)) return false;
  dkd_job_notification_seen_value.add(dkd_safe_key_value);
  if (dkd_job_notification_seen_value.size > 120) {
    const dkd_first_value = dkd_job_notification_seen_value.values().next().value;
    if (dkd_first_value) dkd_job_notification_seen_value.delete(dkd_first_value);
  }
  return true;
}
...
export async function dkd_panel_register_push_token() {
  try {
    ...
    if (!dkd_project_id) return { dkd_ok_value: false, dkd_reason_value: 'panel_remote_push_project_not_configured' };
...
export async function dkd_panel_notify_new_order(dkd_order_value = {}) {
  ...
  return dkd_panel_present_local_notification_value(
    'Yeni Sipariş Geldi',
    dkd_ref_value ? `#${dkd_ref_value} • ${dkd_title_value}` : dkd_title_value,
    { dkd_route_value: 'orders', dkd_job_id_value: dkd_order_value?.id || null, dkd_event_key: 'new_order' },
  );
}
...
export async function dkd_panel_handle_job_realtime_notification(dkd_payload_value = {}) {
  try {
    ...
    if (['completed','delivered','done','finished'].includes(dkd_status_value) || ['delivered','completed'].includes(dkd_pickup_status_value) || dkd_record_value?.is_active === false) {
      dkd_event_key_value = 'delivered';
      dkd_notification_title_value = 'Sipariş Teslim Edildi';
      dkd_notification_body_value = `#${dkd_order_ref_value} • ${dkd_title_value} teslimatı tamamlandı.`;
    } else if (['accepted','assigned','to_pickup'].includes(dkd_status_value)) {
      dkd_event_key_value = 'accepted';
      dkd_notification_title_value = 'Kurye Görevi Kabul Etti';
      dkd_notification_body_value = `#${dkd_order_ref_value} • ${dkd_title_value} kurye tarafından kabul edildi.`;
    } else {
      return { dkd_ok_value: true, dkd_skipped_value: true };
    }

    if (!dkd_remember_notification_key_value(`${dkd_job_id_value}:${dkd_event_key_value}`)) {
      return { dkd_ok_value: true, dkd_duplicate_value: true };
    }
    return dkd_panel_present_local_notification_value(
      dkd_notification_title_value,
      dkd_notification_body_value,
      { dkd_route_value: 'orders', dkd_job_id_value, dkd_event_key: dkd_event_key_value },
    );
  } catch (dkd_error_value) {
    return { dkd_ok_value: false, dkd_reason_value: dkd_error_value?.message || String(dkd_error_value) };
  }
