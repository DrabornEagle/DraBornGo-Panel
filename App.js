// DraBornGo Panel v0.0.3 management center release.
...
import PlatformFeeScreen from './src/screens/PlatformFeeScreen';
import AdminCenterScreen from './src/screens/AdminCenterScreen';
...
import { dkd_panel_fetch_admin_bootstrap } from './src/services/adminService';
...
function DkdAuthenticatedApp({ session }) {
  ...
  const [dkd_is_admin,dkd_set_is_admin] = useState(null);
  ...
  const dkd_load_profile = useCallback(async () => {
    dkd_set_error(''); dkd_set_loading(true);
    try {
      const dkd_admin_value = await dkd_panel_fetch_admin_bootstrap();
      if (dkd_admin_value?.dkd_is_admin === true) {
        dkd_set_is_admin(true); dkd_set_business(null); return;
      }
      dkd_set_is_admin(false);
  ...
  useEffect(() => {
    if (dkd_is_admin !== false) return undefined;
  ...
  if (dkd_loading) return <View style={styles.loading}><ActivityIndicator color={dkd_theme.cyan} size="large" /><Text style={styles.loadingText}>Hesap yetkisi hazırlanıyor…</Text></View>;
  ...
  return <View style={styles.appRoot}>
    ...
    {dkd_tab === 'platform_fee' && <PlatformFeeScreen />}
...
export default function App() {
  ...
  return <SafeAreaProvider><StatusBar style="light" translucent backgroundColor="transparent" /><NavigationBar style="dark" hidden={false} /><SafeAreaView style={styles.safe} edges={['top']}>{!dkd_supabase_ready ? <View style={styles.loading}><MaterialCommunityIcons name="database-alert-outline" size={38} color={dkd_theme.yellow} /><Text style={styles.errorTitle}>Veritabanı ayarı eksik</Text><Text style={styles.errorText}>DraBornGo Ortak Veritabanı bağlantısı yüklenemedi. Güncel GitHub sürümüne eşitle.</Text></View> : dkd_booting ? <View style={styles.loading}><ActivityIndicator size="large" color={dkd_theme.cyan} /></View> : dkd_session ? <DkdAuthenticatedApp session={dkd_session} /> : <AuthScreen />}</SafeAreaView></SafeAreaProvider>;
}

const styles=StyleSheet.create({
  safe:{flex:1,backgroundColor:dkd_theme.background},appRoot:{flex:1,backgroundColor:dkd_theme.background},loading:{flex:1,alignItems:'center',justifyContent:'center',backgroundColor:dkd_theme.background,padding:28,gap:10},loadingText:{color:dkd_theme.textSoft,fontSize:15,fontWeight:'800'},errorTitle:{color:dkd_theme.text,fontSize:22,fontWeight:'900',textAlign:'center'},errorText:{color:dkd_theme.textSoft,fontSize:14.5,lineHeight:21,fontWeight:'700',textAlign:'center',maxWidth:340},retry:{marginTop:10,minHeight:50,minWidth:140,borderRadius:15,backgroundColor:dkd_theme.cyan,alignItems:'center',justifyContent:'center'},retryText:{color:'#07121D',fontSize:14,fontWeight:'900'},logoutMini:{minHeight:44,minWidth:120,alignItems:'center',justifyContent:'center'},logoutMiniText:{color:'#FF9EAA',fontSize:13.5,fontWeight:'900'},navWrap:{position:'absolute',left:8,right:8,bottom:8},navBar:{minHeight:76,borderRadius:26,paddingHorizontal:4,flexDirection:'row',alignItems:'center',backgroundColor:'rgba(8,17,30,.98)',borderWidth:1,borderColor:'rgba(166,217,255,.16)'},navItem:{flex:1,minWidth:0,alignItems:'center',justifyContent:'center',gap:4},navIcon:{width:34,height:34,borderRadius:12,alignItems:'center',justifyContent:'center'},navIconActive:{backgroundColor:dkd_theme.cyan},navText:{color:'#71839C',fontSize:10.5,fontWeight:'900'},navTextActive:{color:'#CFF8FF'}
});
